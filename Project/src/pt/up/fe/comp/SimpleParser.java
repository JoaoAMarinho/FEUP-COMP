...
import pt.up.fe.comp.ast.NodePositionAnnotator;
...
import static pt.up.fe.comp.JmmGrammarConstants.TokenType.INVALID;
...
public class SimpleParser implements JmmParser {
    ...
    public JmmParserResult parse(String jmmCode, String startingRule, Map<String, String> config) {
        ...
        try {
            ...
            new NodePositionAnnotator().visit((JmmNode) root);
            ...
            ParseException pe = TestUtils.getException(e, ParseException.class);
            if (pe != null) {
                Token token = pe.getToken();
                Stage stage = token.getType() == INVALID ? Stage.LEXICAL : Stage.SYNTATIC;
                Report report = Report.newError(stage, token.getBeginLine(), token.getBeginColumn(), pe.getMessage(), pe);
